...
public class GameCarGUI extends GameComponentGUI<Car> {
	...
	public GameCarGUI(Car element) {
		...
		program = GameResourceManager.getInstance().getShaderProgramByName(
				"speed");
		program1=GameResourceManager.getInstance().getShaderProgramByName(
				"speed");
	...
	private void drawQuadrants(float[] proj, float[] look) {
		...
		GLES20.glUseProgram(program1.getProgram());
	
		int mPositionHandle = GLES20.glGetAttribLocation(program1.getProgram(),
				"vPosition");
		...
		int mUvs = GLES20.glGetAttribLocation(program1.getProgram(), "vUvs");
		...
		int mNormal = GLES20.glGetAttribLocation(program1.getProgram(),
				"vNormal");
		...
		int mvpMatrixHandle = GLES20.glGetUniformLocation(program1.getProgram(),
				"uMVPMatrix");
		...
		mTextureUniformHandle = GLES20.glGetUniformLocation(program1.getProgram(),
				"u_Texture");
